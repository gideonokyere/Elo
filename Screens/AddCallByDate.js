...
import {addCallByDates,listDateCalls,deleteCall,listCallDone,callDrop} from '../actions/callAction';

class AddCallByDate extends Component {
...
searchContact=async(names)=>{
  ...
  addCall=()=>{
    ...
    this.props.saveCall(name,number,date);
...
const mapDispatchToProps = (despatch)=>{
  return{
    fetchCalls:()=>despatch(listDateCalls()),
    saveCall:(name,number,date)=>despatch(addCallByDates(name,number,date)),
...
export default connect(mapStateToProps,mapDispatchToProps)(AddCallByDate);